...
d1.subset<-data.frame(date=mdy(d1$DATE_COL),state=d1$ST.x,tn=d1$NTL,tn.flag=NA,tp=d1$PTL,tp.flag=NA,site.type=d1$SITE_TYPE.x,
...
# combine different data quality flags into single column to exclude later
d2$tn.flag<-paste(d2$NTL_FLAG,d2$NTL_QA_FLAG,sep="")
d2$tp.flag<-paste(d2$PTL_QA_FLAG,d2$PTL_FLAG,sep="")

d2.subset<-data.frame(date=mdy(d2$DATE_COL),state=d2$STATE,tn=d2$NTL_RESULT,tn.flag=d2$tn.flag,tp=d2$PTL_RESULT,tp.flag=d2$tp.flag,
                      site.type=d2$SITETYPE,lat=d2$LAT_DD83,lon=d2$LON_DD83,siteID=d2$SITE_ID,weight=d2$WGT_ALL)
...
d3.tn.small<-data.frame(UID=d3.tn$UID,tn=d3.tn$RESULT,tn.flag=d3.tn$NARS_FLAG) # mg/L

d3.tp<-d3[d3$ANALYTE=="PTL",]
d3.tp.small<-data.frame(UID=d3.tp$UID,tp=d3.tp$RESULT,tp.flag=d3.tp$NARS_FLAG) # ug/l

nutrients.together<-merge(d3.tp.small,d3.tn.small,by="UID")

d3.with.site<-merge(nutrients.together,d3.site.info,by="UID")

d3.subset<-data.frame(date=mdy(d3.with.site$DATE_COL),state=d3.with.site$PSTL_CODE,tn=as.numeric(d3.with.site$tn),tn.flag=d3.with.site$tn.flag,
                      tp=as.numeric(d3.with.site$tp),tp.flag=d3.with.site$tp.flag,site.type=d3.with.site$SITETYPE,
...
# discard observations without states or with data quality flags
data<-data[data$state!="None",]
data$tp.flag<-as.factor(data$tp.flag)
data$tn.flag<-as.factor(data$tn.flag)

lake.TN.data<-data[data$tn.flag=="" | is.na(data$tn.flag)==TRUE,]
...
lake.TP.data<-data[data$tp.flag=="" | is.na(data$tp.flag)==TRUE,]
